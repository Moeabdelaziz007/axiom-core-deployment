-- 🗄️ THE HIVE MEMORY - Swarm Protocol Migration
-- 
-- Database migration for agent collaboration system with D1
-- Enables persistent storage for sessions, tasks, reputation, and knowledge
-- 
-- Migration: 0004_swarm_protocol
...
CREATE INDEX IF NOT EXISTS idx_agent_teams_leader ON agent_teams(leader_id);
CREATE INDEX IF NOT EXISTS idx_agent_teams_status ON agent_teams(status);
CREATE INDEX IF NOT EXISTS idx_agent_teams_created_at ON agent_teams(created_at);
...
CREATE INDEX IF NOT EXISTS idx_swarm_knowledge_created_at ON swarm_knowledge(created_at);
...
CREATE INDEX IF NOT EXISTS idx_agent_reputation_last_updated ON agent_reputation(last_updated);
...
CREATE INDEX IF NOT EXISTS idx_conflict_resolutions_team ON conflict_resolutions(team_id);
...
CREATE INDEX IF NOT EXISTS idx_audit_trail_team ON audit_trail(team_id);
...
CREATE INDEX IF NOT EXISTS idx_performance_metrics_team ON performance_metrics(team_id);
CREATE INDEX IF NOT EXISTS idx_performance_metrics_recorded_at ON performance_metrics(recorded_at);

-- Insert initial data for Axiom agents
INSERT OR IGNORE INTO agent_reputation (agent_id, overall_score, collaboration_score, reliability_score, knowledge_score, leadership_score, innovation_score) VALUES
('tajer', 85.0, 88.0, 82.0, 75.0, 90.0, 85.0),
('aqar', 92.0, 85.0, 95.0, 88.0, 78.0, 92.0),
('mawid', 78.0, 82.0, 85.0, 70.0, 88.0, 75.0),
('sofra', 88.0, 90.0, 88.0, 75.0, 92.0, 85.0);

-- Create default incentive system
INSERT OR IGNORE INTO incentive_systems (id, type, name, description, rules, rewards) VALUES (
'swarm_points',
'points',
'Swarm Intelligence Points',
'Points earned through collaboration and knowledge sharing',
'[{"trigger": "task_completed", "amount": 10}, {"trigger": "knowledge_shared", "amount": 15}, {"trigger": "conflict_resolved", "amount": 20}]',
'[{"name": "skill_boost", "cost": 100, "type": "skill"}, {"name": "resource_allocation", "cost": 50, "type": "resource"}]'
);

-- Migration complete
-- This migration creates the complete database schema for the Swarm Protocol
-- All tables are properly indexed for optimal performance
-- Default reputation scores are set for the four main Axiom agents