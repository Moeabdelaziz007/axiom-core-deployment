...
async function setupOutboxPattern(): Promise<void> {
  ...
  try {
    ...
    const indexChecks = [
      ...
      'idx_delivery_event_id',
      'idx_delivery_subscription_id'
    ];
    ...
    for (const indexName of indexChecks) {
      ...
      if (result.rows.length === 0) {
        logWarning(`Index may be missing: ${indexName}`);
      }
    }
    ...
    const testEventId = await paymentsDb.execute({
      sql: `
    ...
    const testSubscriptionId = await paymentsDb.execute({
      sql: `
        INSERT OR IGNORE INTO event_subscriptions (
          subscription_id, consumer_name, event_types, status
        ) VALUES (?, ?, ?, ?)
      `,
      args: [
        'test_subscription',
    ...
    logSuccess('Test subscription created');
    
    // Clean up test subscription
    await paymentsDb.execute({
      sql: 'DELETE FROM event_subscriptions WHERE subscription_id = ?',
      args: ['test_subscription']
    });
    ...
    logInfo('  4. Start the outbox processor: await getOutboxProcessor().start()');
...
if (require.main === module) {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    ...
    log('', 'reset');
    log('Usage:', 'cyan');
    log('  npm run setup:outbox [options]', 'reset');
    log('', 'reset');
    log('Options:', 'cyan');
    ...
    log('', 'reset');
    log('Examples:', 'cyan');
    log('  npm run setup:outbox                # Full setup', 'reset');
    log('  npm run setup:outbox --verify       # Verify only', 'reset');
    log('  npm run setup:outbox --force       # Force setup', 'reset');
    process.exit(0);
  }
